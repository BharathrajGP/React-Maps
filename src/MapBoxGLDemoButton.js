...
// export default MapBoxGLDemoButton;
...
const MapBoxGLDemoButton = () => {
  ...
  useEffect(() => {
    ...
    mapInstance.on('load', () => {
      ...
      mapInstance.on('moveend', () => {
        ...
        setCenter([newCenter.lng, newCenter.lat]);
        setZoom(newZoom);
        setDisableZoomOut(newZoom <= 2); // Disable zoom out button when zoom level is less than or equal to 2
      });
  ...
  useEffect(() => {
    ...
    map.on('moveend', async () => {
      ...
      setMarkerData(response.data.Items);
      console.log("Yesssss",response.data.Items.length);
    });
  }, [map]);
  ...
  const handleZoomIn = () => {
    ...
    const currentZoom = map.getZoom();
    map.setZoom(currentZoom + 1);
    setDisableZoomOut(false); // Re-enable zoom out button
  };
  ...
  const handleZoomOut = () => {
    ...
    const currentZoom = map.getZoom();
    map.setZoom(currentZoom - 1);
    setDisableZoomOut(currentZoom - 1 <= 2); // Disable zoom out button when zoom level is less than or equal to 2
  };
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '600px' }}>
      ...
      <div style={{ position: 'absolute',display:"flex",flexDirection:"column", top: '10px', left: '10px', zIndex: 1 }}>
        <button onClick={handleZoomIn} style={{width:"30px",height:"30px",cursor:"pointer"}}>+</button>
        <button onClick={handleZoomOut} disabled={disableZoomOut} style={{width:"30px",height:"30px",cursor:"pointer"}}>-</button>
      </div>
    </div>
  );
};
